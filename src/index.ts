...
type ResultLikeAwaited<A = never, E = never> = DistributeOk<Ok<A>> | DistributeErr<Err<E>>;
type ResultLikePromise<A = never, E = never> = Promise<ResultLikeAwaited<A, E>>;

export type ResultLike<A, E> = Result<A, E> | ResultLikePromise<A, E> | ResultLikeAwaited<A, E>;
...
export class Result<A = never, E = never> implements PromiseLike<Ok<A> | Err<E>> {
  ...
  // ---- unwrap -----------------------------------------------------------------------------
  static unwrap = <A>(self: ResultLike<A, never>): Promise<A> =>
